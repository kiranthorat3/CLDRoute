#!/usr/bin/env python3
"""
latent_trainer.py — LDM trainer for ldm_control (ControlNet-style conditioning).

N14 updates:
  - reads task and tech from the VAE checkpoint
  - resolves N14/N28 dataset paths automatically
  - keeps all feature channels by default
  - stores tech and z_clip in checkpoints
  - uses symmetric latent clipping in encode/decode/sampling

This is a ControlNet-style multiscale conditioner, not a full literal
ControlNet clone. That is fine as long as it is described honestly.
"""
...
def _load_vae(ckpt_path: str, vae_dir: str, device: torch.device):
    ...
    if stored_vae_dir and os.path.realpath(stored_vae_dir) != os.path.realpath(vae_dir):
        print("  [VAE] WARNING: supplied vae_dir differs from stored:")
...
def _load_latent_stats(ckpt_path: str, latent_ch: int, device: torch.device):
    ...
    if len(z_mean_l) != latent_ch:
        raise ValueError(f"latent_stats channel mismatch: {len(z_mean_l)} vs {latent_ch}")
    if min(z_std_l) < 0.05:
        raise ValueError(
            f"Latent std too small (min={min(z_std_l):.4f}). "
            f"Retrain VAE before LDM."
        )
...
class LatentTrainer:
    def __init__(self, cfg: LatentConfig, resume_ckpt: str | None = None):
        ...
        if cfg.H_latent != 64 or cfg.W_latent != 64:
            raise ValueError(
        ...
        # ── Train mean for congestion trivial baseline ────────────────────────
        if self.task == "Congestion":
            ...
            ldr_full = DataLoader(
                ...
                num_workers=cfg.num_workers, collate_fn=_collate, drop_last=False,
            )
            s, n = 0.0, 0
        ...
        # ── Optimizer + EMA ───────────────────────────────────────────────────
        params = list(self.net.parameters()) + list(self.conditioner.parameters())
        ...
        print("=" * 70)
        print(f"  LDM-Control | task={self.task} | tech={self.tech} | device={self.device}")
        ...
        print("  Conditioner: MultiScale ControlNet-style (64×64, 32×32, 16×16)")
        print(f"  UNet in_ch={cfg.latent_ch} (latent only)")
        ...
        with torch.no_grad():
            ...
            out = self.net(z0, torch.zeros(2, 128, device=self.device), c64=c64, c32=c32, c16=c16)
            out_u = self.net(z0, torch.zeros(2, 128, device=self.device), c64=None, c32=None, c16=None)

        print(f"[Sanity] z_norm mean={z0.mean():.3f} std={z0.std():.3f}")
        print(f"[Sanity] c64={tuple(c64.shape)} c32={tuple(c32.shape)} c16={tuple(c16.shape)}")
        print(f"[Sanity] cond out={tuple(out.shape)} null out={tuple(out_u.shape)}")
        print(f"[Sanity] ZeroConv check: c64_norm={c64.norm():.4f} (should be ~0 at init)")
        print()
        ...
        for epoch in range(self.start_epoch, cfg.epochs + 1):
            ...
            for feat, lbl, _ in self.loader_train:
                ...
                torch.nn.utils.clip_grad_norm_(
                    list(self.net.parameters()) + list(self.conditioner.parameters()),
                    cfg.grad_clip,
                )
                self.optimizer.step()
                self.global_step += 1
        ...
        with _EMASwap(self.ema, self._combined):
            ...
            if self.task == "DRC":
                ...
                print(f"  [E{epoch:03d}] gen({n_gen}) "
                      ...
                      f"nz_MAE={nmae:.5f} [{len(cfg.eval_seeds)} seeds]")
            else:
                ...
                print(f"  [E{epoch:03d}] gen({n_gen}) "
                      ...
                      f"SSIM={ssim:.4f} [{len(cfg.eval_seeds)} seeds]")
