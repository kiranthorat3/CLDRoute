...
N14 update:
  ...
  - no feature channels are dropped by default
  - z_clip is stored explicitly so training/sampling stay consistent

Note:
  This stack is ControlNet-style, not a full literal ControlNet clone.
"""
...
class LatentConfig:
    ...
    # ── Feature dropping ──────────────────────────────────────────────────────
    # Keep all channels by default. Do not drop macro_boundary_distance unless
    # you have evidence it hurts on this specific tech/task.
    drop_feat_channels: List[int] = field(default_factory=list)

    # ── Conditioner / UNet ────────────────────────────────────────────────────
    feat_proj_ch: int = 64   # unused in ldm_control; kept for config compatibility
    ...
    # ── Latent clipping ───────────────────────────────────────────────────────
    # Keep encode/decode/sampling domain consistent.
    z_clip: float = 3.0
