#!/usr/bin/env python3
"""
vae_train_congestion.py — Training for CongestionVAE on CircuitNet-N14.

Loss:
  L1 reconstruction + beta * KL (with free-bits per channel)

Checkpoints:
  best_l1.pt   — lowest val L1
  best_ssim.pt — highest val SSIM
  best_ldm.pt  — lowest val L1 among latent-ready epochs — preferred for LDM

Practical fallback:
  If no epoch passes the latent-readiness gate, downstream handoff falls back
  to best_l1.pt and is written to ldm_handoff.txt.

Usage:
  python vae_train_congestion.py \
    --out_dir ./runs/vae_Cong_N14_splitB_v1 \
    --beta_target 0.005 \
    --epochs 150
"""
...
# ─────────────────────────────────────────────────────────────────────────────
def build_args():
    p = argparse.ArgumentParser("CongestionVAE training (N14)")
...
def main():
    ...
    for attr in [
        "out_dir", "epochs", "batch_size", "lr", "beta_target",
    ...
    print("=" * 65)
    print(f"  CongestionVAE | tech={cfg.tech} | task={cfg.task} | device={device}")
        ...
        if epoch % cfg.eval_every == 0:
            ...
            with torch.no_grad():
                for lbl, _ in loader_val:
                    ...
                    val_l1 += recon_loss_l1(recon, lbl).item() * lbl.shape[0]
                    val_kl += kl_loss(
                        mu, logvar,
                        cfg.logvar_min, cfg.logvar_max,
                        free_bits=cfg.free_bits
                    ...
                    for i in range(gt_np.shape[0]):
                        ...
                        ssim_list.append(float(
                            structural_similarity(gt_np[i], pr_np[i], data_range=1.0)
                        ))
            ...
            if val_ssim > best_ssim:
                ...
                save_ckpt(
                    ...
                    model, ema, optimizer, epoch, cfg,
                    {"best_ssim": best_ssim, "best_ssim_epoch": best_ssim_epoch},
                    global_step
                )
                print(f"  [BEST_SSIM  E{epoch:03d}] "
                      f"val_ssim={best_ssim:.4f} — saved")
            ...
            if ready and val_l1 < best_ldm_l1:
                ...
                print(f"  [BEST_LDM   E{epoch:03d}] "
                      f"L1={best_ldm_l1:.5f} "
                      f"ch_std_min={min_ch_std:.4f} — saved best_ldm.pt")
    ...
    if best_ldm_l1 < float("inf"):
        ...
        print(f"[DONE] best_ldm L1={best_ldm_l1:.5f} @ epoch {best_ldm_epoch}")
        print(f"[DONE] Recommended LDM handoff: {handoff_ckpt}")
    else:
        handoff_ckpt = os.path.join(cfg.out_dir, "best_l1.pt")
        print("[DONE] best_ldm: no epoch passed readiness gate.")
        print("[DONE] Falling back to best_l1.pt for downstream experiments.")
    ...
    print(f"\n[DONE] Next steps:")
    print(f"  python vae_eval_congestion.py --ckpt {handoff_ckpt}")
    print(f"  python vae_latent_stats_congestion.py --ckpt {handoff_ckpt}")
